...
export function SanLuisPotosiTemplate({ preview = false }: { preview?: boolean }) {
  ...
  const propertyLabelMap: Record<string, string> = {
    casa: 'CASA-HABITACIÓN', departamento: 'DEPARTAMENTO',
    local: 'LOCAL COMERCIAL', oficina: 'OFICINA',
  };
  const propertyUsageMap: Record<string, string> = {
    casa: 'USO HABITACIONAL', departamento: 'USO HABITACIONAL',
    local: 'USO COMERCIAL', oficina: 'USO COMUNICACIÓN / OFICINA',
  };
  ...
  return (
    <div className="text-[10px] md:text-[11px] text-gray-800 leading-relaxed space-y-4 text-justify font-serif print:text-black">
      ...
      <div className="text-center font-bold mt-6 mb-4"><h3>D E C L A R A C I O N E S</h3></div>
      ...
      <div className="text-center font-bold mt-8 mb-4"><h3>C L Á U S U L A S</h3></div>
      ...
      <p>ENTERADAS "LAS PARTES" CONTRATANTES DEL VALOR Y ALCANCE LEGAL DE TODAS Y CADA UNA DE LAS CLÁUSULAS EXPRESADAS, FIRMAN DE ENTERA CONFORMIDAD EL PRESENTE CONTRATO EN <span className="font-bold">SAN LUIS POTOSÍ, SAN LUIS POTOSÍ, MÉXICO</span>.</p>
